...
import { SKILLS, calculateSkillTotal, getRemainingPoints, getSkillLimit } from '../data/characterData';
...
const canIncrease = (skill) => {
    // Pega as informações da perícia para saber o atributo base
    const skillInfo = SKILLS.find(s => s.key === skill);
    if (!skillInfo) return false; // Perícia não encontrada

    const currentRanks = character.pericias[skill] || 0;
    
    // Pega o valor do atributo base (ex: Força, Agilidade, etc.)
    const baseAttributeValue = character.atributos[skillInfo.baseAttribute] || 0;

    // Calcula o valor TOTAL da perícia
    const totalSkillValue = baseAttributeValue + currentRanks;
    
    // Pega o limite correto (NP x 2)
    const limit = getSkillLimit(character.nivelPoder || 10);

    // Calcula o custo para adicionar o próximo ponto
    ...
    // CONDIÇÃO CORRIGIDA:
    // Agora ele verifica se o valor TOTAL é menor que o limite
    // E se há pontos restantes para o custo adicional.
    return totalSkillValue < limit && remainingPoints >= additionalCost;
  };
  ...
  return (
    <div className="bg-card p-6 rounded-lg border shadow-sm">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-xl font-bold text-foreground">Perícias</h2>
        <InfoIcon content={`Perícias custam 1 ponto para cada 2 graduações (arredondado para cima). O total é atributo base + graduações. Limite máximo: ${getSkillLimit(character.nivelPoder || 10)} graduações (NP × 2).`} />
      </div>

      {/* Adicionar perícia customizada */}
      <div className="mb-6 p-4 bg-muted rounded-lg border">
        <h3 className="font-semibold text-foreground mb-3">Adicionar Perícia Personalizada</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {SKILLS.map((skill) => {
          ...
          return (
            <div key={skill.key} className="border rounded-lg p-4 bg-muted">
      ...
      <div className="mt-6 bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg">
        <h4 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">Sistema de Perícias</h4>
