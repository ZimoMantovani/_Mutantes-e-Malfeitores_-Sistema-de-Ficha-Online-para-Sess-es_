...
import { DEFENSES, calculateDefenseValue, getRemainingPoints, getDefenseLimit } from '../data/characterData';
...
const DefensesSection = () => {
  ...
  const canIncrease = (defense) => {
    ...
    const baseAttributeValue = character.atributos[DEFENSES.find(d => d.key === defense)?.baseAttribute] || 0;
    const totalValue = 10 + baseAttributeValue + currentValue;
    const limit = getDefenseLimit(character.nivelPoder || 10);
    return totalValue < limit && remainingPoints >= 1;
  };
  ...
  return (
    <div className="bg-card p-6 rounded-lg border shadow-sm">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-xl font-bold text-foreground">Defesas</h2>
        <InfoIcon content={`Defesas custam 1 ponto por nível de bônus. O valor total é 10 + atributo base + bônus comprado. Limite máximo: ${getDefenseLimit(character.nivelPoder || 10)} (NP × 2).`} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {DEFENSES.map((def) => {
          ...
          return (
            <div key={def.key} className="border rounded-lg p-4 bg-background">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground">
                  Limite: {getDefenseLimit(character.nivelPoder || 10)}
                </div>
                {totalValue >= getDefenseLimit(character.nivelPoder || 10) && (
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg">
          <h4 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">Defesas Ativas</h4>
        ...
        <div className="bg-green-50 dark:bg-green-950/20 p-3 rounded-lg">
          <h4 className="font-semibold text-green-800 dark:text-green-200 mb-2">Defesas de Resistência</h4>
